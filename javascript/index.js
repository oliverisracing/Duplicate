...
function miniChallenge() {
    ...
    let challenges = ["actingChallenge()", "improvChallenge()", "marketingChallenge()", "danceChallenge()", "designChallenge()", "comedyChallenge()", "singingChallenge()", "runwayChallenge()"];
    ...
    if (danceChallengeCounter == 3 && totalCastSize > 15 || danceChallengeCounter == 2 && totalCastSize <= 15)
...
class SingingChallenge {
    generateDescription() {
        ...
        let desc1;
        (function (desc1) {
            desc1[desc1["Rocker chicks"] = 0] = "Rocker chicks";
            desc1[desc1["RuPaul-a-Palooza"] = 1] = "RuPaul-a-Palooza";
            desc1[desc1["Frenemies"] = 2] = "Frenemies";
            desc1[desc1["Can I Get An Amen?"] = 3] = "Can I Get An Amen?";
            desc1[desc1["Spoof! (There It Is)"] = 4] = "Spoof! (There It Is)";
            desc1[desc1["Divine Inspiration"] = 5] = "Divine Inspiration";
            desc1[desc1["I'm That Bitch"] = 6] = "I'm That Bitch";
            desc1[desc1["Condragulations"] = 7] = "Condragulations";
            desc1[desc1["Phenomenon"] = 8] = "Phenomenon";
        })(desc1 || (desc1 = {}));
        description.innerHTML = "The queens will participate in a singing number called " + desc1[randomNumber(0, 8)];
        if (randomNumber(0, 100) >= 50) {
            episodeChallenges.push("Singing");
        } else {
            episodeChallenges.push("Music");
        }
    }
    rankPerformances() {
        for (let i = 0; i < currentCast.length; i++)
            currentCast[i].getSinging();
        sortPerformances(currentCast);
    }
}
...
function lipsyncsLC() {
    if ((top4 || lftc || canFinale || teamsF || allstars3Finale) && eliminatedCast.length == Math.floor((totalCastSize - 4) / 2) && !team && !lipsyncChallengeCounter && lip15sync || top5 && eliminatedCast.length == Math.floor((totalCastSize - 5) / 2) && !lipsyncChallengeCounter && lip15sync || top3 && eliminatedCast.length == Math.floor((totalCastSize - 3) / 2) && !lipsyncChallengeCounter && lip15sync) {
        let screen = new Scene();
        if (!solidbk) {
...
function createChallenge(challenges, miniChallengeScreen) {
    ...
    //lipsync challenge
    else if ((top4 || lftc || canFinale || teamsF || allstars3Finale) && eliminatedCast.length == Math.floor((totalCastSize - 4) / 2) && !team && !lipsyncChallengeCounter && lip15sync || top5 && eliminatedCast.length == Math.floor((totalCastSize - 5) / 2) && !lipsyncChallengeCounter && lip15sync || top3 && eliminatedCast.length == Math.floor((totalCastSize - 3) / 2) && !lipsyncChallengeCounter && lip15sync)
    ...
    miniChallengeScreen.createButton("Proceed", "girlgroup()");
    //rusical
    else if (totalCastSize >= 12 && (currentCast.length == 11 || currentCast.length == 9) && !rusicalCounter && randomNumber(0, 100) >= 50 || totalCastSize < 12 && currentCast.length == 7 && !rusicalCounter  && randomNumber(0, 100) >= 40 || currentCast.length > 5 && randomNumber(0, 20) >= 10 && team && !rusicalCounter)
...
function checkForDeptQuit(pana) {
    if (pana) {
        if (randomNumber(0, 1000) >= 999 && !disqOrDept) {
            let screen = new Scene();
...
function votingChart() {
    ...
    for (let i = 0; i < episodeChallenges.length - 1; i++) {
        if (!(episodeChallenges[i] == "Lipsync Smackdown" || episodeChallenges[i] == "LaLaPaRUza" || episodeChallenges[i] == "Queens of Comedy" || episodeChallenges[i] == "Kitty Girl Group" || episodeChallenges[i] == "Lipsync" || episodeChallenges[i] == "Lip Sync")) {
    ...
    if (onFinale) {
        ...
        for (let i = 0; i < winnerQueen.trackRecord.length + 1; i++) {
            ...
            if (placement.innerHTML == "WIN") {
                ...
                placement.setAttribute("style", "background-color: #A9A9A9;");
            }
...
function lsticksChoices() {
    ...
    let tha3 = document.createElement("th");
        ...
        for (let i = 0; i < assasintable.length; i++) {
            ...
            i++;
            if (namea1.innerHTML == " "){
                namea1.setAttribute("style", "background-color: #A9A9A9;");
                lipstickkk.setAttribute("style", "background-color: #A9A9A9;");
            }
...
else if (document.getElementById("background").checked == true){
    solidbk = true;
}
if (team && (chocolateBarTwist || immunityTwist || lip15sync)) {
...
function startSimulation(challenge = "") {
    ...
    if (currentCast.length == 0)
        ...
        randomReturn = true;
            ...
            smackdown = true;
                ...
                if (team || all_winners) {
                    ...
                    let randomNumberReturn = randomNumber(0, 7);
        ...
        else if (team && (smackdown || immunityTwist || lip15sync || voteReturn || randomReturn || chooseReturn || chocolateBarTwist || s9Premiere || s6Premiere || lalaparuza || queensOfComedy || kittyGirlGroup || conjoinedQueens || s12Premiere || porkchopPremiere || s14Premiere || uk3Premiere || top5 || top4 || top3 || lftc || canFinale || allstars3Finale)) {
        ...
        else if(all_winners && (smackdown || s14Premiere || s12Premiere || s9Premiere || s6Premiere || porkchopPremiere || uk3Premiere || voteReturn || conjoinedQueens || queensOfComedy || kittyGirlGroup || randomReturn || chooseReturn || lalaparuza || chocolateBarTwist || top5 || top4 || top3 || canFinale || allstars3Finale) || all_winners && currentCast < 8) {
            window.alert("The All Winners Format isn't avaliable with any combination of premiere, returning challenge or Chocolate Bar Twist, at this moment. Finale format must be Lipsync for the Crown. It needs at least 8 contestants.");
            s6Premiere = false;
            s9Premiere = false;
            s12Premiere = false;
            s14Premiere = false;
...
class Queen {
    ...
    getActing() {
        this.performanceScore = this._calculateScores(20, 30, this._actingStat);
    }
    getComedy() {
        this.performanceScore = this._calculateScores(20, 30, this._comedyStat);
    }
    getMarketing() {
        this.performanceScore = this._calculateScores(45, 50, this._comedyStat + this._actingStat + this._brandingStat);
    }
    getDance() {
        this.performanceScore = this._calculateScores(20, 30, this._danceStat);
    }
    getSinging()  {
        this.performanceScore = this._calculateScores(20, 30, this._singingStat);
    }
    getDesign() {
        this.performanceScore = this._calculateScores(20, 30, this._designStat);
    }
    getRunwayChallenge() {
        this.performanceScore = this._calculateScores(20, 30, this._runwayStat);
    }
    getImprov() {
        this.performanceScore = this._calculateScores(20, 30, this._improvStat);
    }
    //special 'gets':
    getSnatch() {
        this.performanceScore = this._calculateScores(40, 50, this._improvStat + this._comedyStat + this._actingStat);
    }
    getRusical() {
...
let tammies1 = new Queen("Tammie Brown", 8, 9, 10, 5, 6, 8, 6, 14, 15,"TammieBrownS1");
...
let jessicaw = new Queen("Jessica Wild", 10, 10, 7, 11, 9, 10, 5, 6, 14,"Jessica");
let jujus2 = new Queen("Jujubee", 8, 9, 12, 8, 7, 6, 15, 10, 13,"JujubeeS2");
let morganmcs2 = new Queen("Morgan McMichaels", 8, 7, 6, 10, 13, 10, 11, 8, 7, "MorganS2");
let mystique = new Queen("Mystique Summers Madison", 6, 8, 9, 11, 7, 5, 10, 8, 6, "Mystique");
...
let sahara = new Queen("Sahara Davenport", 8, 6, 8, 14, 5, 8, 14, 10, 7,"Sahara");
let shangela = new Queen("Shangela", 8, 6, 8, 7, 2, 5, 5, 10, 6, "ShangelaS2");
let kylies2 = new Queen("Kylie Sonique Love", 8, 7, 5, 8, 10, 10, 13, 8, 6,"SoniqueS2");
let tatis2 = new Queen("Tatianna", 8, 10, 12, 8, 5, 8, 9, 5, 10, "TatiannaS2");
...
let manilas3 = new Queen("Manila Luzon",10,13,9,10,11,17,13,11,13,"ManilaS3",);
...
let rajas3 = new Queen("Raja",9,11,10,8,20,13,10,5,17,"RajaS3");
let shangelas3 = new Queen("Shangela",10,11,11,8,2,7,11,10,7,"ShangelaS3");
...
let yaras3 = new Queen("Yara Sofia",10,9,7,10,12,11,9,8,12,"YaraS3");
...
let jiggly = new Queen("Jiggly Caliente",9,4,6,10,2,6,13,4,6,"JigglyS4");
let kenya = new Queen("Kenya Michaels",6,4,6,7,5,6,6,8,6,"Kenya");
let lashauwn = new Queen("Lashauwn Beyond",6,4,7,6,9,10,8,6,6,"Lashauwn");
let latrice = new Queen("Latrice Royale",12,5,9,12,5,6,14,6,14,"LatriceS4");
let madamel = new Queen("Madame LaQueer",4,7,7,10,7,8,7,6,6,"Madame");
...
let kelly = new Queen("Kelly Mantle",8,6,8,6,3,6,5,8,3,"Kelly");
...
//SEASON 7: 
let ginger = new Queen("Ginger Minj",12,14,13,10,6,6,10,13,13,"GingerS7");
let jaidynn = new Queen("Jaidynn Diore Fierce",5,5,8,9,7,7,13,8,10,"Jaidynn");
...
let katya = new Queen("Katya Zamolodchikova",11,11,10,10,7,9,8,13,9,"KatyaS7");
let kennedy = new Queen("Kennedy Davenport",5,13,12,13,7,7,15,10,14,"KennedyS7");
let max = new Queen("Max",14,6,10,7,8,12,3,12,11,"Max");
let fame = new Queen("Miss Fame",6,6,6,5,10,14,5,8,7,"Fame");
let kasha = new Queen("Mrs. Kasha Davis",6,7,6,7,7,8,6,8,9,"Kasha");
let pearl = new Queen("Pearl Liaison",8,8,10,4,10,13,6,7,7,"Pearl");
let frisbee = new Queen("Frisbee Jenkins",5,5,7,4,5,6,6,5,3,"Sasha");
let tempest = new Queen("Tempest DuJour",5,4,3,3,3,6,4,6,3,"Tempest");
let trixie = new Queen("Trixie Mattel",8,8,9,3,8,8,5,11,7,"TrixieS7");
let violet = new Queen("Violet Chachki",9,9,8,10,20,15,7,10,10,"Violet");
...
let acid = new Queen("Acid Betty",9,6,9,10,12,13,6,8,10,"Acid");
let btdq = new Queen("Bob The Drag Queen",14,14,14,10,6,8,11,14,11,"Bob");
let chichis8 = new Queen("Chi Chi DeVayne",8,7,7,14,7,8,15,4,9,"ChiChiS8");
let cynthialees8 = new Queen("Cynthia Lee Fontaine",4,4,5,5,6,5,6,7,3,"CynthiaS8");
let dax = new Queen("Dax Exclamation Point",4,5,6,3,7,10,3,7,4,"Dax");
let derricks8 = new Queen("Derrick Barry",6,5,8,5,6,5,9,12,8,"DerrickS8");
let kimchi = new Queen("Kim Chi",9,9,8,4,15,13,2,8,9,"Kim");
let lailamcqueen = new Queen("Laila McQueen",4,4,6,4,4,8,6,8,5,"Laila");
let naomis8 = new Queen("Naomi Smalls",9,6,9,12,9,13,11,11,11,"NaomiS8");
let naysha = new Queen("Naysha Lopez",5,6,6,4,4,7,5,7,6,"Naysha");
let robbie = new Queen("Robbie Turner",5,5,8,10,6,6,5,10,14,"Robbie");
...
//SEASON 9: 
let aja = new Queen("Aja LaBeija",6,8,5,10,4,8,13,10,9,"AjaS9");
let alexism = new Queen("Alexis Michelle",10,10,10,10,3,6,11,8,10,"Alexis");
let charlie = new Queen("Charlie Hides",4,4,6,4,6,13,0,7,3,"Charlie");
let cynthialees9 = new Queen("Cynthia Lee Fontaine",5,4,6,5,6,5,6,7,5,"CynthiaS9");
let eureka = new Queen("Eureka",7,8,8,8,7,11,11,8,7,"EurekaS9");
let farrah = new Queen("Farrah Moan",6,5,5,4,4,10,7,9,5,"FarrahS9",);
let jaymes = new Queen("Jaymes Mansfield",4,4,3,3,4,7,4,8,3,"Jaymes");
let kimora = new Queen("Kimora Blac",4,5,6,4,5,6,8,6,6,"Kimora");
let nina = new Queen("Nina Bo'nina Brown",6,12,8,7,10,12,12,6,6,"Nina");
let peppermint = new Queen("Peppermint",8,6,13,12,7,7,15,10,12,"Peppermint");
let sasha = new Queen("Sasha Velour",8,12,12,7,9,12,15,14,10,"SashaVelour");
let sheas9 = new Queen("Shea Coulée",12,12,12,13,12,12,7,14,18,"SheaS9");
...
let eurekas10 = new Queen("Eureka O'Hara",7,13,17,7,8,11,11,15,6,"EurekaS10");
...
let vanessas10 = new Queen("Vanessa Vanjie Matteo",4,5,4,5,3,3,7,7,8,"VanessaS10");
let yuhua = new Queen("Yuhua Hamasaki",7,6,6,6,8,10,6,5,6,"Yuhua");
...
let yvie = new Queen("Yvie Oddly",12,10,7,14,13,11,15,11,12,"YvieS11");
let brooke = new Queen("Brooke Lynn Hytes", 8,11,8,11,15,20,15,9,12,"Brooke");
...
let vanjies11 = new Queen("Vanessa Vanjie Matteo",9,5,9,5,7,8,10,10,7,"VanessaS11");
...
let daya = new Queen("Daya Betty", 10, 10, 8, 8, 8, 10, 7, 9, 12,"Daya");
...
let jorgeous = new Queen("Jorgeous", 8, 8, 10, 15, 8, 8, 15, 12, 7,"Jorgeous");
let junej = new Queen("June Jambalaya", 5, 5, 4, 3, 3, 8, 10, 8, 2,"June");
let kerri = new Queen("Kerri Colby", 6, 6, 4, 7, 5, 8, 5, 10, 7,"Kerri");
let kornbread = new Queen("Kornbread \"The Snack\" Jeté", 8, 9, 8, 6, 6, 8, 8, 12, 11,"Kornbread");
let camden = new Queen("Lady Camden", 14, 10, 10, 11, 10, 10, 12, 8, 13,"Camden");
let maddym = new Queen("Maddy Morphosis", 7, 6, 6, 6, 1, 6, 7, 6, 3,"Maddy");
let orion = new Queen("Orion Story", 6, 5, 4, 3, 3, 3, 6, 6, 5,"Orion");
let willow = new Queen("Willow Pill", 10, 11, 11, 8, 14, 10, 12, 12, 10,"Willow");
...
let anetra = new Queen("Anetra", 8, 9, 9, 13, 11, 12, 12, 13, 11,"Anetra");
let auraMayari = new Queen("Aura Mayari", 6, 6, 6, 9, 6, 8, 7, 6, 5,"AuraMayari");
let irene = new Queen("Irene Dubois", 5, 5, 5, 5, 5, 3, 5, 5, 3,"IreneDubois");
let jax = new Queen("Jax", 6, 6, 6, 6, 6, 6, 14, 6, 6,"Jax");
let loosey = new Queen("Loosey LaDuca", 11, 10, 12, 7, 6, 8, 8, 11, 8,"LooseyLaDuca");
let luxx = new Queen("Luxx Noir London", 8, 10, 13, 10, 17, 9, 14, 8, 9,"LuxxNoirLondon");
...
let mistress = new Queen("Mistress Isabelle Brooks", 14, 10, 13, 7, 11, 10, 10, 9, 9,"MistressIsabelleBrooks");
...
let sashaColby = new Queen("Sasha Colby", 18, 10, 10, 15, 13, 12, 12, 15, 15,"SashaColby");
let spice = new Queen("Spice", 7, 7, 6, 8, 9, 15, 8, 8, 9,"Spice");
let sugar = new Queen("Sugar", 7, 7, 7, 7, 7, 13, 7, 7, 7,"Sugar");
...
//ALL STARS 2
let adoreas2 = new Queen("Adore Delano", 9, 10, 11, 7, 6, 11, 9, 10, 10,"AdoreAS2")
let alaskaas2 = new Queen("Alaska Thunderfuck", 14, 11, 10, 8, 10, 10, 10, 14, 9,"AlaskaAS2");
let alyssaeas2 = new Queen("Alyssa Edwards", 7, 8, 12, 14, 8, 5, 14, 15, 15,"AlyssaAS2");
let cocoas2 = new Queen("Coco Montrese", 8, 9, 8, 14, 12, 10, 13, 7, 9,"CocoAS2");
let detoxas2 = new Queen("Detox", 9, 10, 13, 10, 11, 13, 12, 10, 13,"DetoxAS2");
let gingeras2 = new Queen("Ginger Minj",13, 11, 10, 8, 9, 4, 8, 9, 8,"GingerAS2");
let katyaas2 = new Queen("Katya", 11, 12, 13, 10, 11, 10, 10, 10, 9,"KatyaAS2");
let jaremyas2 = new Queen("Jaremy Carey", 8, 10, 11, 9, 13, 15, 10, 11, 8,"JaremyAS2");
let roxxxyas2 = new Queen("Roxxxy Andrews", 8, 10, 7, 10, 9, 14, 15, 10, 6,"RoxxxyAS2");
let tatias2 = new Queen("Tatianna", 10, 13, 10, 12, 9, 8, 15, 10, 9,"TatiannaAS2");
let allstars_2 = [adoreas2, alaskaas2, alyssaeas2, cocoas2, detoxas2, gingeras2, katyaas2, jaremyas2, roxxxyas2, tatias2];
//ALL STARS 3
let ajaas3 = new Queen("Aja Labeija", 10, 11, 12, 13, 12, 12, 10, 11, 13,"AjaAS3");
let bebeas3 = new Queen("Bebe Zahara Benet", 9, 8, 10, 11, 9, 7, 13, 14, 16,"BebeAS3");
let delaas3 = new Queen("BenDeLaCreme",12, 12, 12, 12, 12, 12, 8, 12, 15,"BendelaAS3");
let chichias3 = new Queen("Chi Chi Devayne", 9, 10, 8, 14, 7, 9, 15, 10, 9,"ChiChiAS3");
let kennedyas3 = new Queen("Kennedy Davenport", 9, 10, 7, 12, 8, 10, 15, 8, 9,"KennedyAS3");
let milkas3 = new Queen("Milk", 7, 7, 7, 9, 9, 12, 9, 6, 7,"MilkAS3");
let morganas3 = new Queen("Morgan McMichaels", 6, 6, 9, 7, 7, 9, 12, 6, 7,"MorganAS3");
let shangelaas3 = new Queen("Shangela", 11, 12, 13, 12, 6, 10, 12, 12, 11,"ShangelaAS3");
let thorgy = new Queen("Thorgy Thor", 10, 11, 8, 7, 12, 11, 7, 8, 9,"ThorgyAS3");
let trixieas3 = new Queen("Trixie Mattel", 10, 8, 9, 7, 8, 9, 7, 10, 8,"TrixieAS3")
let allstars_3 = [ajaas3, bebeas3,delaas3,chichias3,kennedyas3,milkas3,morganas3,shangelaas3, thorgy,trixieas3];
//ALL STARS 4
let farrahas4 = new Queen("Farrah Moan", 6, 5, 4, 5, 8, 10, 8, 6, 2,"FarrahAS4");
let giagas4 = new Queen("Gia Gunn", 6, 5, 8, 8, 9, 10, 13, 8, 3,"GiaAS4");
let jasmineas4 = new Queen("Jasmine Masters", 5, 6, 6, 6, 6, 6, 9, 7, 6,"JasmineAS4");
let latriceas4 = new Queen("Latrice Royale", 10, 11, 12, 10, 8, 7, 9, 8, 11,"LatriceAS4");
let manilaas4 = new Queen("Manila Luzon", 10, 12, 13, 12, 11, 8, 14, 13, 9,"ManilaAS4");
let monetas4 = new Queen("Monét X Change", 10, 11, 13, 10, 7, 9, 14, 11, 14,"MonetAS4");
let moas4 = new Queen("Mo Heart", 12, 11, 13, 10, 9, 12, 10, 11, 6,"MoAS4");
let naomias4 = new Queen("Naomi Smalls", 9, 10, 10, 12, 14, 13, 15, 11, 11,"NaomiAS4");
let tttas4 = new Queen("Trinity The Tuck", 12, 13, 14, 11, 11, 14, 10, 8, 9,"TrinityAS4");
let valentinas4 = new Queen("Valentina", 10, 9, 8, 9, 12, 11, 15, 11, 17,"ValentinaAS4");
let allstars_4 = [farrahas4, giagas4, jasmineas4, latriceas4, manilaas4, monetas4, moas4, naomias4, tttas4, valentinas4];
//ALL STARS 5
let alexisas5 = new Queen("Alexis Mateo", 12, 11, 10, 10, 11, 8, 12, 10, 11,"AlexisAS5");
let blairas5 = new Queen("Blair St. Clair", 9, 10, 7, 8, 13, 12, 9, 8, 10,"BlairAS5");
let derrickas5 = new Queen("Derrick Barry", 10, 8, 12, 10, 5, 7, 10, 7, 8,"DerrickAS5");
let indiaas5 = new Queen("India Ferrah", 7, 7, 7, 7, 7, 10, 12, 7, 3,"IndiaAS5");
let jujubeeas5 = new Queen("Jujubee", 11, 10, 9, 11, 8, 13, 10, 10, 11,"JujubeeAS5");
let mariahas5 = new Queen("Mariah Paris Balenciaga", 9, 10, 7, 9, 7, 13, 11, 9, 8,"MariahAS5");
let mayhemas5 = new Queen("Mayhem Miller", 10, 11, 12, 8, 9, 11, 13, 8, 9,"MayhemAS5");
let mizas5 = new Queen("Miz Cracker", 12, 11, 14, 10, 9, 9, 7, 12, 8,"MizAS5");
let onginaas5 = new Queen("Ongina", 8, 7, 9, 7, 10, 12, 6, 7, 7,"OnginaAS5");
let sheaas5 = new Queen("Shea Coulée", 10, 11, 13, 14, 12, 13, 14, 10, 16,"SheaAS5");
let allstars_5 = [alexisas5, blairas5, derrickas5, indiaas5, jujubeeas5, mariahas5, mayhemas5, mizas5, onginaas5, sheaas5];
//ALL STARS 6
let akeriaas6 = new Queen("Akeria C. Davenport", 10, 11, 7, 9, 12, 15, 10, 8, 9,"AkeriaAS6");
let eurekaas6 = new Queen("Eureka!", 12, 11, 13, 12, 10, 9, 11, 8, 11,"EurekaAS6");
let gingeras6 = new Queen("Ginger Minj", 13, 14, 15, 9, 8, 5, 13, 12, 11,"GingerAS6");
let janas6 = new Queen("Jan", 8, 9, 8, 10, 12, 12, 4, 10, 10,"JanAS6");
let jigglyas6 = new Queen("Jiggly Caliente", 10, 9, 11, 9, 5, 7, 10, 6, 5,"JigglyAS6");
let kylieas6 = new Queen("Kylie Sonique Love", 12, 11, 13, 12, 10, 13, 14, 9, 13,"KylieAS6");
let pandoraas6 = new Queen("Pandora Boxx", 11, 12, 9, 7, 6, 8, 7, 10, 8,"PandoraAS6");
let rajahas6 = new Queen("Ra'Jah O'Hara", 12, 11, 13, 12, 11, 13, 15, 12, 11,"RajahAS6");
let scarletas6 = new Queen("Scarlet Envy", 9, 11, 10, 8, 9, 13, 8, 12, 9,"ScarletAS6");
let serenaas6 = new Queen("Serena ChaCha ", 6, 6, 7, 10, 9, 13, 8, 6, 3,"SerenaAS6");
let silkyas6 = new Queen("Silky Nutmeg Ganache", 12, 11, 9, 10, 9, 8, 15, 10, 9,"SilkyAS6");
let trinityas6 = new Queen("Trinity K. Bonet ", 12, 11, 8, 13, 12, 13, 10, 11, 16,"TrinityAS6");
let yaraas6 = new Queen("Yara Sofia", 9, 10, 11, 13, 11, 12, 10, 11, 10,"YaraAS6");
...
//ALL STARS 7
let jaidaas7 = new Queen("Jaida Essence Hall", 10, 11, 10, 12, 20, 14, 12, 13, 13,"JaidaAS7");
let jinkxas7 = new Queen("Jinkx Monsoon", 14, 12, 15, 10, 11, 11, 13, 12, 13,"JinkxAS7");
let monetas7 = new Queen("Monét X Change", 11, 12, 13, 14, 10, 12, 14, 11, 20,"MonetAS7");
let rajaas7 = new Queen("Raja", 14, 11, 12, 11, 14, 13, 12, 11, 13,"RajaAS7");
let sheaas7 = new Queen("Shea Coulée", 11, 12, 13, 12, 12, 14, 14, 13, 25,"SheaAS7");
let vivas7 = new Queen("The Vivienne", 14, 12, 11, 10, 13, 12, 14, 13, 15,"VivienneAS7");
let trinityas7 = new Queen("Trinity The Tuck", 14, 14, 11, 13, 20, 14, 12, 12, 11,"TrinityAS7");
let yvieas7 = new Queen("Yvie Oddly", 12, 11, 10, 15, 12, 12, 14, 15, 15,"YvieAS7");
let allstars_7 = [jaidaas7, jinkxas7, monetas7, rajaas7, sheaas7, vivas7, trinityas7, yvieas7];
//DRUK SEASON 1 
let viv = new Queen("The Vivienne", 10, 11, 12, 6, 11, 12, 10, 10, 9,"VivienneS1");
let blus1 = new Queen("Blu Hydrangea", 8, 10, 11, 13, 8, 10, 9, 9, 10,"BluS1");
...
let divina = new Queen("Divina De Campo", 11, 12, 8, 13, 12, 12, 8, 10, 16,"Divina");
let gothy = new Queen("Gothy Kendoll", 8, 6, 7, 6, 10, 3, 6, 8, 3,"Gothy");
let scaredy = new Queen("Scaredy Kat", 8, 7, 9, 8, 5, 14, 7, 6, 2,"Scaredy");
let sum = new Queen("Sum Ting Wong", 10, 8, 7, 6, 8, 10, 10, 12, 6,"Sum");
let vinegar = new Queen("Vinegar Strokes", 12, 11, 10, 9, 4, 6, 8, 9, 5,"Vinegar");
let bagas1 = new Queen("Baga Chipz", 11, 12, 14, 8, 4, 6, 8, 15, 14,"BagaS1");
let uk_season1 = [viv, blus1, cheryls1, crystal, divina, gothy, scaredy, sum, vinegar, bagas1];
//DRUK SEASON 2
let lawrence = new Queen("Lawrence Chaney", 10, 12, 8, 8, 13, 12, 10, 14, 12,"Lawrence");
let bimini = new Queen("Bimini", 12, 10, 11, 10, 6, 12, 12, 13, 18,"Bimini");
let tayce = new Queen("Tayce", 8, 7, 9, 8, 6, 12, 15, 10, 15,"Tayce");
let ellie = new Queen("Ellie Diamond", 8, 9, 10, 8, 12, 10, 8, 10, 8,"Ellie");
let awhora = new Queen("A'Whora", 8, 12, 6, 8, 14, 14, 10, 10, 12,"Awhora");
let sister = new Queen("Sister Sister", 8, 7, 9, 8, 10, 10, 10, 7, 6,"Sister"); 
let tia = new Queen("Tia Kofi", 9, 10, 7, 6, 6, 8, 13, 10, 7,"Tia");
let joe = new Queen("Joe Black", 8, 9, 10, 6, 7, 8, 5, 10, 8,"Joe");
let veronicas2 = new Queen("Veronica Green", 10, 6, 8, 10, 8, 10, 10, 8, 9,"VeronicaS2");
let ginny = new Queen("Ginny Lemon", 8, 6, 8, 6, 4, 5, 7, 3, 5,"Ginny");
let asstina = new Queen("Asttina Mandela", 9, 10, 9, 8, 4, 10, 13, 10, 11,"Asttina");
let cherry = new Queen("Cherry Valentine", 6, 7, 8, 7, 12, 14, 7, 8, 5,"Cherry");
let uk_season2 = [lawrence,bimini,tayce,ellie,awhora,sister,tia,joe, veronicas2, ginny, asstina, cherry];
//DRUK SEASON 3    
let krystal = new Queen("Krystal Versace", 11, 10, 6, 12, 13, 15, 14, 11, 9,"Krystal");
let ella = new Queen('Ella Vaday', 11, 10, 12, 7, 11, 12, 5, 8, 13,"Ella");
let kitty = new Queen("Kitty Scott-Claus", 12, 11, 12, 8, 8, 9, 10, 10, 9,"Kitty");
let vanity = new Queen('Vanity Milan', 8, 9, 8, 12, 7, 10, 15, 10, 12,"VanityS3");
let scarletth = new Queen('Scarlett Harlett', 9, 10, 7, 8, 14, 10, 13, 10, 3,"ScarlettS3");
...
let charity = new Queen('Charity Kase', 8, 9, 7, 7, 13, 15, 9, 12, 6,"Charity");
let veronicas3 = new Queen('Veronica Green', 9, 10, 11, 8, 6, 7, 13, 10, 10,"VeronicaS3");
let victorias = new Queen('Victoria Scone', 9, 10, 12, 8, 9, 13, 9, 10, 8,"VictoriaS3");
let elektraf = new Queen("Elektra Fence", 5, 6, 8, 10, 7, 8, 14, 10, 4,"Elektra");
let anubis = new Queen("Anubis Finch", 6, 7, 8, 7, 9, 7, 9, 10, 3,"Anubis");
...
let baby = new Queen("Baby", 6, 6, 9, 13, 9, 9, 14, 7, 12,"Baby");
let black = new Queen("Black Peppa", 6, 4, 8, 9, 6, 13, 14, 7, 10,"BlackPeppa");
let cheddar = new Queen("Cheddar Gorgeous", 12, 10, 10, 8, 13, 14, 8, 12, 12,"Cheddar");
let copper = new Queen("Copper Topp", 5, 4, 9, 7, 6, 7, 8, 7, 10,"Copper");
let dakota = new Queen("Dakota Schiffer", 6, 9, 9, 9, 10, 11, 10, 8, 15,"Dakota");
let danny = new Queen("Danny Beard", 12, 10, 11, 6, 8, 12, 9, 10, 10,"Danny");
let jonbers = new Queen("Jonbers Blonde", 5, 10, 8, 7, 9, 9, 9, 7, 5,"Jonbers");
let just = new Queen("Just May", 4, 4, 4, 4, 4, 4, 4, 3, 3,"Just");
let leFil = new Queen("Le Fil", 6, 5, 8, 9, 5, 12, 8, 5, 7,"LeFil");
let pixie = new Queen("Pixie Polite", 7, 5, 9, 5, 6, 5, 9, 9, 7,"Pixie");
let sminty = new Queen("Sminty Drop", 5, 6, 5, 8, 9, 14, 8, 6, 10,"Sminty");
...
//CAN SEASON 1 
let anastarzia = new Queen("Anastarzia Anaquway", 7, 6, 8, 7, 9, 8, 6, 8, 6,"Anastarzia");
let boa = new Queen("BOA", 7, 9, 9, 7, 6, 5, 5, 9, 11,"BOA");
let ilona = new Queen("Ilona Verley", 6, 7, 9, 7, 9, 8, 11, 8, 5,"Ilona");
let jimbo = new Queen("Jimbo", 12, 10, 14, 4, 8, 7, 3, 9, 7,"JimboS1");
let juice = new Queen("Juice Boxx", 7, 6, 7, 6, 5, 9, 6, 7, 2,"Juice");
let kiara = new Queen("Kiara",8, 9, 6, 7, 10, 8, 7, 5, 9,"Kiara");
let kyne = new Queen("Kyne", 7, 8, 6, 8, 7, 6, 6, 8, 7,"Kyne");
let lemon = new Queen("Lemon", 11, 9, 8, 10, 6, 13, 12, 8, 11, "LemonS1");
let priyanka = new Queen("Priyanka", 8, 12, 13, 10, 6, 8, 15, 13, 16,"Priyanka");
let rita = new Queen("Rita Baga", 8, 10, 12, 6, 10, 8, 12, 8, 10,"RitaS1");
let scarlettbobo = new Queen("Scarlett BoBo", 12, 10, 8, 8, 10, 11, 13, 12, 11,"ScarlettBoBo");
let tynomi = new Queen("Tynomi Banks", 6, 7, 9, 12, 8, 8, 13, 9, 8,"Tynomi");
...
let kendall = new Queen("Kendall Gender", 7, 9, 10, 8, 6, 10, 14, 10, 6,"KendallS2");
let eve = new Queen("Eve 6000", 6, 7, 6, 4, 6, 10, 6, 8, 7,"Eve");
let giam = new Queen("Gia Metric", 9, 10, 12, 14, 7, 10, 10, 6, 14,"GiaMetric");
let pythia = new Queen("Pythia", 10, 11, 8, 7, 10, 10, 5, 12, 9,"Pythia");
let adriana = new Queen("Adriana", 13, 8, 6, 7, 9, 12, 9, 12, 13,"Adriana");
let kimoraa = new Queen("Kimora Amour", 7, 6, 7, 7, 6, 8, 7, 7, 6,"KimoraAmour");
let synthia = new Queen("Synthia Kiss", 11, 8, 12, 10, 5, 8, 13, 10, 12,"Synthia");
let suki = new Queen("Suki Doll", 7, 8, 7, 8, 12, 15, 6, 9, 6,"Suki");
let oceane = new Queen("Océane Aqua-Black", 6, 3, 3, 3, 3, 3, 10, 6, 5,"Oceane");
let beth = new Queen("Beth", 4, 3, 4, 4, 5, 4, 8, 3, 4,"Beth");
let stephanie = new Queen("Stephanie Prince", 6, 5, 5, 5, 10, 12, 10, 8, 7,"StephanieS2");
...
let vanderpuss = new Queen("Vivian Vanderpuss", 9, 9, 9, 7, 8, 9, 8, 9, 10, "Vivian");
...
let envy = new Queen("Envy Peru", 12,11,11,12,8,15,12,12,13,"Envy");
let janeys1 = new Queen("Janey Jacké", 9,9,9,12,10,12,12,8,12,"JaneyS1");
...
let roem = new Queen("Roem",6,6,6,4,7,3,6,4,6,"Roem");
...
//DRT SEASON 2
let angele = new Queen("Angele Anang", 11,11,11,10,13,15,15,8,10,"Angele");
let bandit = new Queen("Bandit",12,8,10,13,8,13,12,9,7,"Bandit");
let genie = new Queen("Genie", 11,10,10,8,6,13,7,10,7,"Genie");
let gimhuay = new Queen("Miss Gimhuay", 10,10,9,8,12,15,8,8,6,"Gimhuay");
let kana = new Queen("Kana Warrior", 6,7,7,10,9,12,15,10,10,"Kana");
...
let maya = new Queen("Maya B'Haro", 6,6,7,8,10,11,8,7,9,"Maya");
let mocha = new Queen("Mocha Diva", 12,12,8,10,10,6,8,10,6,"Mocha");
let silvers = new Queen("Silver Sonic", 5,5,5,6,8,9,6,6,10,"Silver");
let srimala = new Queen("Srimala",8,4,6,8,7,11,15,8,6,"Srimala");
let tormai = new Queen("Tormai",8,9,7,8,5,10,10,7,7,"Tormai");
let vanda = new Queen("Vanda Miss Joaquim", 12,7,7,13,7,11,15,7,10,"Vanda");
...
let arts = new Queen("Art Simone", 10, 8, 7, 10, 14, 15, 6, 10, 9,"ArtSimone");
let cocoj = new Queen("Coco Jumbo", 8, 10, 9, 8, 7, 6, 12, 6, 6,"CocoJumbo");
let elektra = new Queen("Elektra Shock", 11, 10, 12, 15, 9, 4, 14, 10, 10,"ElektraShock");
let etc = new Queen("Etcetera Etcetera", 8, 9, 10, 10, 12, 14, 12, 10, 9,"Etc");
let jojoz = new Queen("Jojo Zaho", 7, 7, 6, 7, 9, 6, 8, 10, 7,"Jojo");
let karenf = new Queen("Karen From Finance", 11, 10, 9, 6, 5, 12, 9, 15, 9,"Karen");
let kita = new Queen("Kita Mean", 12, 12, 13, 14, 8, 12, 10, 15, 10,"Kita");
let maxi = new Queen("Maxi Shield", 8, 9, 7, 10, 12, 10, 15, 9, 7,"Maxi");
...
let hannah = new Queen("Hannah Conda", 11, 10, 8, 9, 9, 11, 8, 10, 12, "Hannah");
let kweenKong = new Queen("Kween Kong", 5, 9, 10, 5, 8, 10, 11, 10, 12, "Kween");
...
let spankie = new Queen("Spankie Jackzon", 11, 11, 8, 5, 8, 7, 9, 9, 12, "Spankie");
let yuri = new Queen("Yuri Guaii", 6, 11, 6, 12, 9, 13, 7, 8, 7, "Yuri");
let drdu_season2 = [aubrey, beverly, faux, hannah, kweenKong, minnie, molly, pomara, spankie, yuri];
//DRAG RACE ESPAÑA 1
let carmenf = new Queen("Carmen Farala",10, 11, 10, 8, 13, 15, 14, 12, 16, "CarmenF");
let killer = new Queen("Killer Queen", 8, 11, 13, 8, 9, 13, 12, 10, 9, "Killer");
let sagi = new Queen("Sagittaria", 10, 11, 8, 12, 13, 15, 12, 13, 9,"Sagittaria");
let pupi = new Queen("Pupi Poisson", 14, 11, 8, 10, 6, 7, 8, 11, 11,"Pupi");
let dovima = new Queen("Dovima Nurmi", 7, 8, 7, 9, 7, 10, 12, 8, 9,"Dovima");
let hug = new Queen("Hugáceo Crujiente", 7, 8, 7, 9, 13, 14, 9, 8, 9,"Hugaceo");
let arantxa = new Queen("Arantxa Castilla La Mancha", 11, 8, 6, 12, 6, 8, 10, 8, 7,"Arantxa");
let inti = new Queen("Inti", 8, 9, 7, 8, 10, 11, 10, 12, 8,"Inti");
let dragv = new Queen("Drag Vulcano", 8, 9, 6, 7, 10, 11, 6, 14, 7,"DragVulcano");
...
let venedita = new Queen("Venedita Von Däsh", 9, 13, 10, 8, 8, 12, 10, 8, 10,"Venedita");
let juriji = new Queen("Juriji Der Klee", 12, 7, 7, 9, 10, 10, 9, 12, 12,"Juriji");
let sethlas = new Queen("Drag Sethlas", 8, 9, 7, 8, 12, 10, 8, 12, 9,"DragSethlas");
...
let sharonne = new Queen("Sharonne", 12, 10, 8, 8, 12, 10, 9, 14, 16,"Sharonne");
...
let elecktra = new Queen("Elecktra Bionic", 8, 13, 9, 12, 10, 10, 11, 11, 10,"ElecktraBionic");
let enorma = new Queen("Enorma Jean", 11, 11, 10, 5, 9, 9, 5, 11, 10,"Enorma");
let farida = new Queen("Farida Kant", 9, 5, 11, 12, 14, 13, 10, 9, 11,"Farida");
let ivana = new Queen("Ivana Vamp", 9, 9, 10, 6, 6, 7, 5, 7, 10,"Ivana");
let leriche = new Queen("Le Riche",9, 14, 9, 12, 11, 12, 10, 6, 8,"LeRiche");
let luquisha = new Queen('Luquisha Lubamba',7, 10, 10, 9, 2, 4, 8, 7, 12,"Luquisha");
...
//DRAG RACE FRANCE
let elips = new Queen("Elips", 7, 9, 8, 8, 8, 8, 7, 6, 6, "Elips");
...
let paloma = new Queen("Paloma", 11, 11, 6, 7, 9, 9, 8, 11, 8, "Paloma");
let soa = new Queen("Soa de Muse", 9, 10, 10, 8, 8, 9, 10, 10, 14, "Soa");
...
let marinaSummers = new Queen("Marina Summers", 6, 9, 12, 9, 10, 12, 10, 13, 10, "MarinaSummers");
...
let turing = new Queen("Turing", 6, 6, 9, 6, 6, 7, 9, 4, 11, "Turing");
let vinas = new Queen("Viñas DeLuxe", 6, 8, 8, 10, 8, 11, 7, 8, 9, "Viñas");
let xilhouete = new Queen("Xilhouete", 6, 10, 6, 8, 11, 10, 8, 10, 10, "Xilhouete");
let drph = [brigiding, corazon, eva, gigiEra, morgana, marinaSummers, minty, precious, prince, turing, vinas, xilhouete];
//UKVSTW
let bagauk = new Queen("Baga Chipz", 11, 10, 14, 7, 6, 7, 9, 12, 10,"BagaUKVSTW");
let bluuk = new Queen("Blu Hydrangea", 10, 11, 13, 9, 8, 14, 12, 11, 11,"BluTW");
let cheryluk = new Queen("Cheryl Hole", 11, 12, 9, 8, 7, 8, 9, 10, 9,"CherylUKVSTW");
let janeyuk = new Queen("Janey Jacké", 10, 11, 8, 15, 13, 10, 11, 9, 12,"JaneyUKVSTW"); 
let jimbouk = new Queen("Jimbo", 14, 13, 12, 7, 13, 12, 4, 10, 9,"JimboUKVSTW");
let jujubee = new Queen("Jujubee", 9, 11, 12, 9, 6, 7, 13, 11, 10,"JujubeeUKVSTW");
let lemonuk = new Queen("Lemon", 10, 8, 9, 13, 9, 14, 13, 10, 8,"LemonUKVSTW");
let mouk = new Queen("Mo Heart", 12, 12, 13, 11, 13, 15, 9, 10, 10,"MoUKVSTW");
...
//CVSTW
let rajahcan = new Queen("Rajah O Hara", 10, 11, 13, 11, 10, 12, 13, 8, 10,"RajahCVSTW");
let victoriacan = new Queen("Victoria Scone", 13, 12, 13, 9, 11, 9, 11, 11, 8,"VictoriaCVSTW");
let vanitycan = new Queen("Vanity Milan", 9, 8, 14, 9, 10, 12, 14, 10, 16, "VanityCVSTW");
let ritacan = new Queen("Rita Baga", 12, 11, 8, 7, 10, 9, 10, 11, 9, "RitaCVSTW");
let icesiscan = new Queen("Icesis Couture", 11, 13, 13, 15, 12, 15, 14, 13, 13, "IcesisCVSTW");
let anitacan = new Queen("Anita Wiglit", 10, 8, 7, 8, 10, 10, 8, 11, 7, "AnitaCVSTW");
let kendallcan = new Queen("Kendall Gender", 7, 11, 10, 6, 8, 9, 10, 8, 8, "KendallCVSTW");
let stephaniecan = new Queen("Stephanie Prince", 5, 6, 3, 8, 6, 12, 10, 8, 7,"StephanieCVSTW");
let silkycan = new Queen("Silky Nutmet Ganache", 13, 12, 11, 10, 11, 9, 15, 13, 11, "SilkyCVSTW");
...
function returningQueenScreen() {
    ...
    screen.createButton("Proceed", "newEpisode()");
    if (voteReturn || (randomReturn || chooseReturn) && document.querySelector("button[onclick='fijarReturningQueen()']") || queensOfComedy && document.querySelector("button[onclick='queensofComedyJudging(pairQOF, qofcomedy)']") || kittyGirlGroup && document.querySelector("button[onclick='kittygirlGroupJudging()']")){
...
function untucked() {
    ...
    if ((all_stars || lipsync_assassin) && isLSShowable) {
        isLSShowable = false;
